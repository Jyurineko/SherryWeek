...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`header\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`logo_id\` integer,
  	\`site_title\` text DEFAULT '赛博莉莉丝' NOT NULL,
  	\`tagline\` text DEFAULT '探索技术，分享生活',
  	\`updated_at\` text,
  	\`created_at\` text,
  ...
  await db.run(sql`CREATE TABLE \`footer\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`copyright\` text DEFAULT '© 2026 赛博莉莉丝. All rights reserved.',
  	\`icp\` text,
  	\`show_powered_by\` integer DEFAULT true,
  	\`updated_at\` text,
  	\`created_at\` text
  );
  `)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_categories\`("id", "name", "slug", "description", "updated_at", "created_at") SELECT "id", "name", "slug", "description", "updated_at", "created_at" FROM \`categories\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "categories_id", "tags_id", "posts_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "categories_id", "tags_id", "posts_id" FROM \`payload_locked_documents_rels\`;`)
